...
t_fdf	*read_map_data(t_fdf *data, char *file)
{
	...
	if (allocate_map_memory(data) == -1)
		return (free_map(data->dmap.input_map, data->dmap.height,
				data->dmap.width), free(data), close(fd), NULL);
	...
	while (i < data->dmap.height)
	{
		...
		while (j < data->dmap.width)
		{
			...
			if (!data->dmap.input_map[i][j])
				return (free_map(data->dmap.input_map, i,
						data->dmap.width), -1);
			j++;
		}
		i++;
...
static int	parse_z_and_color(int *row, char *data)
{
	char	**z_and_color;

	z_and_color = ft_split(data, ',');
	if (!z_and_color)
		return (-1);
	row[0] = ft_atoi(z_and_color[0]);
	if (z_and_color[1])
		row[1] = ft_atoi_base(z_and_color[1]);
	else
		row[1] = -1;
...
int	*parse_map_line(int **row, int column_count, char *line_content)
{
	...
	{
		if (parse_z_and_color(row[i], number[i]) == -1)
		{
			free_arr(number);
			return (NULL);
		}
